...
use polkadot_core_primitives::{AccountId, Balance, Block, BlockNumber, Hash, Nonce};
...
>(
    ...
    if role.is_authority() {
        ...
        > = match sealing {
            ...
            Sealing::Manual => {
                ...
                // We wait for slot duration here to make sure, we have strictly increasing slot numbers
                // otherwise babe block import will reject the block.
                // There might be more efficient way of doing this with stream throttling, but
                // for development purposes it is okay.
                let wrapped_stream = stream.then(move |item| {
                    futures_time::task::sleep(slot_duration.as_duration().into())
                        .then(|_| futures::future::ready(item))
                });
    ...
    let rpc_extensions_builder = {
        ...
        move |deny_unsafe,
            ...
            create_rpc_extension(deps).map_err(Into::into)
        }
    };

...
The payload is consumed by `external-validators` and the validator ids contained in the message are set as external validators. The message processor is `SymbioticInboundMessageProcessorV1` which calls `ExternalRewards::set_external_validators`.

...
#[cfg(feature = "full-node")]
use polkadot_service::overseer::{ExtendedOverseerGenArgs, OverseerGen, OverseerGenArgs};
...
pub use chain_spec::{GenericChainSpec, MozartChainSpec};
...
pub trait IdentifyVariant {
    /// Returns if this is a configuration for the `Mozart` network.
    fn is_mozart(&self) -> bool;
...
impl IdentifyVariant for Box<dyn ChainSpec> {
    fn is_mozart(&self) -> bool {
        self.id().starts_with("mozart") || self.id().starts_with("rco")
    }
    fn is_dev(&self) -> bool {
...
>(
    ...
    // && (config.chain_spec.is_polkadot() || config.chain_spec.is_kusama())
    {
        ...
        if config.chain_spec.is_mozart() || config.chain_spec.is_dev() {
...
/// Build a full node.
///
/// The actual "flavor", aka if it will use `Mozart` is determined based on

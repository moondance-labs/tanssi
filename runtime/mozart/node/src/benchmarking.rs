	...
	(
		...
		match $chain {
			Chain::Mozart => {
				...
				{
					use mozart_runtime as runtime;

					let call = $generic_code;

					Ok(mozart_sign_call(call, $nonce, $current_block, $period, $genesis, $signer))
				}

				#[cfg(not(feature = "mozart-native"))]
				{
					Err("`mozart-native` feature not enabled")
				}
			},
			Chain::Unknown => {
				let _ = $nonce;
...
#[cfg(feature = "mozart-native")]
fn mozart_sign_call(
	call: mozart_runtime::RuntimeCall,
	nonce: u32,
	current_block: u64,
	period: u64,
...
/// Generates inherent data for benchmarking Mozart.
///

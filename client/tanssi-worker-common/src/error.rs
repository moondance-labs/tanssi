...
impl fmt::Display for PrepareError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            RenameTmpFile { err, src, dest } => write!(
                f,
                "prepare: error renaming tmp file ({:?} -> {:?}): {}",
                src, dest, err
            ),
...
pub enum InternalValidationError {
    ...
    /// Could not open IPC Ext socket
    CouldNotOpenIpcExtSocket(String),
...
impl fmt::Display for InternalValidationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            HostCommunication(err) => write!(
                f,
                "validation: some communication error occurred with the host: {}",
                err
            ),
            ...
            CouldNotOpenFile(err) => write!(
                f,
                "validation: could not find or open compiled artifact file: {}",
                err
            ),
            CouldNotOpenIpcExtSocket(err) => write!(
                f,
                "validation: could not find or open IPC ext socket file: {}",
                err,
            ),

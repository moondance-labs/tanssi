...
fn recv_worker_handshake(stream: &mut UnixStream) -> io::Result<WorkerHandshake> {
    let worker_handshake = framed_recv_blocking(stream).map_err(|e| {
    ...
    let worker_handshake = WorkerHandshake::decode(&mut &worker_handshake[..]).map_err(|e| {
        io::Error::new(
            io::ErrorKind::Other,
            format!(
                "recv_worker_handshake: failed to decode WorkerHandshake: {}",
                e
            ),
        )
    })?;
...
pub mod thread {
    ...
    pub fn wait_for_threads(cond: Cond) -> WaitOutcome {
        let (lock, cvar) = &*cond;
        let guard = cvar
            .wait_while(lock.lock().unwrap(), |flag| flag.is_pending())
            .unwrap();
        *guard
    }
    ...
    /// returns `None`.
    #[cfg_attr(
        not(any(target_os = "linux", feature = "jemalloc-allocator")),
        allow(dead_code)
    )]

...
use tanssi_worker_common::{SecurityStatus, WorkerHandshake};
use tokio::{
    ...
    Fut: futures::Future<Output = Result<T, SpawnErr>> + 'static,
{
    ...
    pub async fn tmppath(prefix: &str) -> io::Result<PathBuf> {
        ...
        Err(io::Error::new(
            io::ErrorKind::Other,
            "failed to create a temporary path",
        ))
    }
...
impl WorkerHandle {
    fn spawn(
        ...
        let child_id = child.id().ok_or(io::Error::new(
            io::ErrorKind::Other,
            "could not get id of spawned process",
        ))?;
        let stdout = child
            .stdout
            .take()

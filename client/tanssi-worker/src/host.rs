...
pub const EXECUTE_BINARY_NAME: &str = "tanssi-execute-worker";
...
pub struct ValidationHost<T> {
    to_host_tx: mpsc::Sender<ToHost<T>>,
    /// Available security features, detected by the host during startup.
    pub security_status: SecurityStatus,
}

impl<T> Clone for ValidationHost<T> {
    fn clone(&self) -> Self {
        Self {
            to_host_tx: self.to_host_tx.clone(),
            security_status: self.security_status.clone(),
        }
    }
}

impl<T> ValidationHost<T> {
    ...
    where
        T: From<(IpcExtRequest, oneshot::Sender<IpcExtResponse>)>,
    {
        self.to_host_tx
            .send(ToHost::ExecuteExport(ExecuteExportInputs {
                pvf,
                exec_timeout,
                export_name,
                call_data,
...
enum ToHost<T> {
    ...
    ExecuteExport(ExecuteExportInputs<T>),
    HeadsUp {
        active_pvfs: Vec<PvfPrepData>,
    },
}

struct ExecuteExportInputs<T> {
    pvf: PvfPrepData,
    exec_timeout: Duration,
    export_name: String,
    call_data: Vec<u8>,
...
pub async fn start<T>(
    config: Config,
    metrics: Metrics,
) -> SubsystemResult<(ValidationHost<T>, impl Future<Output = ()>)>
where
    T: From<(
            IpcExtRequest,
...
struct AwaitingPrepare<T>(HashMap<ArtifactId, Vec<PendingExecutionRequest<T>>>);

impl<T> Default for AwaitingPrepare<T> {
    fn default() -> Self {
        Self(Default::default())
    }
}

impl<T> AwaitingPrepare<T> {
    fn add(
        &mut self,
        artifact_id: ArtifactId,
        pending_execution_request: PendingExecutionRequest<T>,
    ) {
        self.0
            .entry(artifact_id)
            .or_default()
            .push(pending_execution_request);
    }

    fn take(&mut self, artifact_id: &ArtifactId) -> Vec<PendingExecutionRequest<T>> {
...
async fn handle_to_host<T>(
    artifacts: &mut Artifacts,
    prepare_queue: &mut mpsc::Sender<prepare::ToQueue>,
    execute_queue: &mut mpsc::Sender<execute::ToQueue<T>>,
    awaiting_prepare: &mut AwaitingPrepare<T>,
    to_host: ToHost<T>,
) -> Result<(), Fatal> {
    match to_host {
        ...
        ToHost::ExecuteExport(inputs) => {
            handle_execute_export(
                artifacts,
                prepare_queue,
                execute_queue,
                awaiting_prepare,
...
// TODO: I removed handle_execute_pvf because it won't compile anymore
...
async fn handle_execute_export<T>(
    artifacts: &mut Artifacts,
    prepare_queue: &mut mpsc::Sender<prepare::ToQueue>,
    execute_queue: &mut mpsc::Sender<execute::ToQueue<T>>,
    awaiting_prepare: &mut AwaitingPrepare<T>,
    inputs: ExecuteExportInputs<T>,
) -> Result<(), Fatal> {
    let ExecuteExportInputs {
        pvf,
        exec_timeout,
        export_name,
        call_data,
...
async fn handle_prepare_done<T>(
    artifacts: &mut Artifacts,
    execute_queue: &mut mpsc::Sender<execute::ToQueue<T>>,
    awaiting_prepare: &mut AwaitingPrepare<T>,
    from_queue: prepare::FromQueue,
) -> Result<(), Fatal> {
    let prepare::FromQueue {
        artifact_id,
        result,
    } = from_queue;
    ...
    let num_failures = if let ArtifactState::Preparing {
        waiting_for_response,
        num_failures,
    } = state
    {
...
async fn send_execute<T>(
    execute_queue: &mut mpsc::Sender<execute::ToQueue<T>>,
    to_queue: execute::ToQueue<T>,
) -> Result<(), Fatal> {
...
async fn enqueue_prepare_for_execute<T>(
    prepare_queue: &mut mpsc::Sender<prepare::ToQueue>,
    awaiting_prepare: &mut AwaitingPrepare<T>,
    pvf: PvfPrepData,
    priority: Priority,
...
pub(crate) mod tests {
    ...
    async fn pruning() {
        ...
        host.heads_up(vec![PvfPrepData::from_discriminator(1)])
            .await
            .unwrap();
    ...
    async fn execute_pvf_requests() {
        ...
        result_tx_pvf_1_1
            .send(Err(ValidationError::PossiblyInvalid(

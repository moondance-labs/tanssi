    ...
    fn all_errs_allowed(&self) -> bool {
        !self.partial.secure_validator_mode
            || self
                .errs
                .iter()
                .all(|err| err.is_allowed_in_secure_mode(&self.partial))
    }
    ...
    fn errs_string(&self) -> String {
        self.errs
            ...
            .map(|err| {
                format!(
                    "\n  - {}{}",
                    if err.is_allowed_in_secure_mode(&self.partial) {
                        "Optional: "
                    } else {
                        ""
                    },
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "linux")] {
                ...
                .await
            {
                ...
                Ok(output) => {
                    let abi =
                        tanssi_worker_common::worker::security::landlock::LANDLOCK_ABI as u8;
                    let stderr = std::str::from_utf8(&output.stderr)
                        .expect("child process writes a UTF-8 string to stderr; qed")
                        .trim();
                    if stderr.is_empty() {

...
use tanssi_worker_common::SecurityStatus;
...
#[derive(Debug)]
pub struct PendingExecutionRequest<T> {
    pub exec_timeout: Duration,
    pub export_name: String,
    pub call_data: Vec<u8>,
    pub executor_params: ExecutorParams,
    pub result_tx: ResultSender,
    pub response_sender: std::sync::mpsc::Sender<T>,
}
...
enum QueueEvent<T> {
    Spawn(IdleWorker, WorkerHandle, ExecuteJob<T>),
    StartWork(Worker, Outcome, ArtifactId, ResultSender),
}

type Mux<T> = FuturesUnordered<BoxFuture<'static, QueueEvent<T>>>;

struct Queue<T> {
    metrics: Metrics,

    /// The receiver that receives messages to the pool.
    to_queue_rx: mpsc::Receiver<ToQueue<T>>,
        ...
        + 'static,
{
    ...
    fn try_assign_next_job(&mut self, finished_worker: Option<Worker>) {
        ...
        // the eldest one.
        let eldest = if let Some(eldest) = self.queue.get(0) {
            eldest
        } else {
            return;
        };
        ...
        if worker.is_none() {
            ...
            worker = self
                .workers
                .find_available(self.queue[job_index].executor_params.hash());
        }
...
fn handle_to_queue<T>(queue: &mut Queue<T>, to_queue: ToQueue<T>)
where
    T: From<(
            IpcExtRequest,
            futures::channel::oneshot::Sender<IpcExtResponse>,
...
async fn handle_mux<T>(queue: &mut Queue<T>, event: QueueEvent<T>)
where
    T: From<(
            IpcExtRequest,
            futures::channel::oneshot::Sender<IpcExtResponse>,
        ...
        + 'static,
{
    ...
    let (idle_worker, result, duration) = match outcome {
        Outcome::Ok {
            ...
            (Some(idle_worker), Ok(encoded_result), Some(duration))
        }
        Outcome::InvalidCandidate { err, idle_worker } => (
            Some(idle_worker),
            Err(ValidationError::Invalid(
                InvalidCandidate::WorkerReportedInvalid(err),
            )),
            None,
        ),
        ...
        Outcome::JobDied { err } => (
            None,
            Err(ValidationError::PossiblyInvalid(
                PossiblyInvalidError::AmbiguousJobDeath(err),
            )),
            None,
        ),
        Outcome::JobError { err } => (
            None,
            Err(ValidationError::PossiblyInvalid(
                PossiblyInvalidError::JobError(err),
            )),
            None,
        ),
    };
...
fn spawn_extra_worker<T>(queue: &mut Queue<T>, job: ExecuteJob<T>)
where
    T: Send + 'static,
{
...
fn assign<T>(queue: &mut Queue<T>, worker: Worker, job: ExecuteJob<T>)
where
    T: From<(
            IpcExtRequest,
            futures::channel::oneshot::Sender<IpcExtResponse>,
...
pub fn start<T>(
    ...
    node_version: Option<String>,
    security_status: SecurityStatus,
) -> (mpsc::Sender<ToQueue<T>>, impl Future<Output = ()>)
where
    T: From<(
            IpcExtRequest,
            futures::channel::oneshot::Sender<IpcExtResponse>,

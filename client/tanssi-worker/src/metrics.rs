...
use tanssi_worker_common::prepare::MemoryStats;
...
impl Metrics {
    ...
    pub(crate) fn prepare_worker(&'_ self) -> WorkerRelatedMetrics<'_> {
        WorkerRelatedMetrics {
            metrics: self,
            flavor: WorkerFlavor::Prepare,
        }
    }
    ...
    pub(crate) fn time_preparation(
        ...
        self.0
            .as_ref()
            .map(|metrics| metrics.preparation_time.start_timer())
    }
    ...
    pub(crate) fn time_execution(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.execution_time.start_timer())
    }
...
impl<'a> WorkerRelatedMetrics<'a> {
    ...
    pub(crate) fn on_begin_spawn(&self) {
        if let Some(metrics) = &self.metrics.0 {
            metrics
                .worker_spawning
                .with_label_values(&[self.flavor.as_label()])
                .inc();
        }
    }

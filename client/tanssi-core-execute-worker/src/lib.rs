...
pub use tanssi_worker_common::{executor_interface::execute_artifact_export, worker_dir};

// NOTE: Initializing logging in e.g. tests will not have an effect in the workers, as they are
//       separate spawned processes. Run with e.g. `RUST_LOG=tanssi-execute-worker=trace`.
const LOG_TARGET: &str = "tanssi-execute-worker";
...
use tanssi_worker_ancillary::{
    recv_vectored_with_ancillary_from, send_vectored_with_ancillary_to, AncillaryData,
    SocketAncillary,
};
...
fn recv_execute_handshake(stream: &mut UnixStream) -> io::Result<Handshake> {
    // TODO: send_execute_handshake is in another crate, polkadot-node-core-pvf
    // Need to implement a new send_exec_request there (the existing send_request will not work)
    let handshake_enc = framed_recv_blocking(stream).map_err(|e| {
        // TODO: framed_recv_blocking should probably return io::Error
...
fn recv_exec_request(stream: &mut UnixStream) -> io::Result<ExecuteRequest> {
    let request_enc = framed_recv_blocking(stream).map_err(|e| {
        // TODO: framed_recv_blocking should probably return io::Error
        io::Error::new(io::ErrorKind::Other, e)
    })?;
    let request = ExecuteRequest::decode(&mut &request_enc[..]).map_err(|_| {
        io::Error::new(
            io::ErrorKind::Other,
            "execute pvf recv_exec_request: failed to decode ExecuteRequest".to_string(),
        )
    })?;
    Ok(request)
}
...
pub fn send_pipes_using_ancillary_magic(
    stream: &mut Option<UnixStream>,
    fds: Vec<RawFd>,
) -> io::Result<()> {
    // TODO: Send a msg like ExpectAncillary?
    //framed_send_blocking(stream, &response.encode())
    let mut ancillary_buffer = [0; 128];
    let mut ancillary = SocketAncillary::new(&mut ancillary_buffer[..]);
    ancillary.add_fds(&fds[..]);
    // We are not transmitting any additional data, only 1 magic byte
    let bufs = vec![IoSlice::new(&[0xAA])];
    let owned_fd_stream = OwnedFd::from(stream.take().unwrap());
    let socket_stream = tanssi_worker_ancillary::Socket(owned_fd_stream);

    send_vectored_with_ancillary_to(&socket_stream, None, &bufs, &mut ancillary)?;

    *stream = Some(UnixStream::from(socket_stream.0));

    Ok(())
}

pub fn recv_pipes_using_ancillary_magic(stream: &mut Option<UnixStream>) -> io::Result<Vec<RawFd>> {
    // TODO: not sure what should these bufs be
    // Buf of size 2 because we are going to receive 2 fds, a PipeReader and a PipeWriter
    let mut bufs = vec![IoSliceMut::new(&mut []), IoSliceMut::new(&mut [])];
    let owned_fd_stream = OwnedFd::from(stream.take().unwrap());
    let socket_stream = tanssi_worker_ancillary::Socket(owned_fd_stream);
...
// random number generator for debugging only
/*
static mut RNG: u64 = 1;
...
fn rng_next() -> u64 {
    ...
    unsafe {
        ...
        RNG ^= RNG << 25;
        RNG ^= RNG >> 27;
        RNG = RNG.wrapping_mul(MULTIPLIER);
        RNG
    }
}

fn rng_seed<T: std::hash::Hash>(s: &T) {
    ...
    // Ensure the seed is not zero to avoid degenerate state.
    unsafe {
        RNG = if hash == 0 { 1 } else { hash };
    }
}
*/
...
pub fn worker_entrypoint(
    ...
    run_worker(
        ...
        |mut stream, worker_dir_path| {
            ...
            // Seed PRNG with something that should be quite random
            //rng_seed(&(&worker_pid, &socket_path));

            // Setup custom panic handler that sends a message to parent process in case of panic.
            // Very unsafe, absolutely do not use unless needed for debugging.
            /*
            let stream_ptr = &stream as *const UnixStream as usize;
            std::panic::set_hook(Box::new(move |panic_info| {
                let response = WorkerResponse::InternalError(
                    InternalValidationError::HostCommunication(format!("panic: {:?}", panic_info)),
                );
                let stream_unsafe_mut_ptr = stream_ptr as *mut UnixStream;
                let stream_unsafe_mut_ref: &mut UnixStream = unsafe { std::mem::transmute(stream_unsafe_mut_ptr) };
                send_response(stream_unsafe_mut_ref, response).unwrap();
            }));
             */
            ...
            loop {
                /*
                // Indeed, this loop runs for more than 1 iteration.
                // Also here after send_response it is missing a `break`, to avoid a deadlock
                // because the ipc_stream will still be open, so secure_executor.rs will be waiting
                // forever. (need to implement a timeout there)
                loop_iterations += 1;
                if rng_next() % 4 == 0 {
                    let response = WorkerResponse::InternalError(
                        InternalValidationError::Kernel(format!("worker iterations: {}, worker dir: {:?}", loop_iterations, worker_dir_path.display())),
                    );
                    send_response(&mut stream, response)?;
                }
                 */
                // Before receiving the request, let's setup the pipe between the child process and
                // secure_executor.rs
                // This must be a new pipe for every request because we spawn a new child process
                // for every request
                let (ipc_pipe_reader1, ipc_pipe_writer1) = os_pipe::pipe()?;
                let (ipc_pipe_reader2, ipc_pipe_writer2) = os_pipe::pipe()?;

                // Send one end of each pipe to secure_executor.rs
                let send_fds = vec![
                    ipc_pipe_reader1.into_raw_fd(),
                    ipc_pipe_writer2.into_raw_fd(),
                ];

                let mut some_stream = Some(stream);
                send_pipes_using_ancillary_magic(&mut some_stream, send_fds)?;
                stream = some_stream.unwrap();

                let ExecuteRequest {
                    export_name,
                    params,
                    execution_timeout,
                } = recv_exec_request(&mut stream)?;
                gum::debug!(
                    target: LOG_TARGET,
                    %worker_pid,
                ...
                if rng_next() % 4 == 0 {
                    let response = WorkerResponse::InternalError(
                        InternalValidationError::Kernel(format!("worker says hi 1, worker dir: {:?}\nsocket path: {:?}\nipc_ext_path: {:?}", worker_dir_path.display(), socket_path.display(), OsString::from_vec(ipc_ext_path.clone()))),
                    );
                    send_response(&mut stream, response)?;
                }
                 */
                ...
                let response = match unsafe { nix::unistd::fork() } {
                    ...
                    Ok(ForkResult::Parent { child }) => {
                        ...
                        drop(pipe_writer);
                        drop(ipc_pipe_reader2);
                        drop(ipc_pipe_writer1);
...
fn execute_export_using_artifact(
...
fn handle_child_process(
    ...
    ipc_pipe_reader: PipeReader,
    ipc_pipe_writer: PipeWriter,
) -> ! {
    ...
    .unwrap_or_else(|err| {
        send_child_response(
            &mut pipe_write,
            Err(JobError::CouldNotSpawnThread(err.to_string())),
        )
    });
    ...
    let response = match outcome {
        WaitOutcome::Finished => {
            ...
            execute_thread
                .join()
                .map_err(|e| JobError::Panic(stringify_panic_payload(e)))
        }
...
fn handle_parent_process(
    ...
    match status {
        Ok(WaitStatus::Exited(_, exit_status)) => {
            ...
            match result {
                Ok(JobResponse::Ok { encoded_result }) => {
...
fn recv_child_response(received_data: &mut io::BufReader<&[u8]>) -> io::Result<JobResult> {
    let response_bytes = framed_recv_blocking(received_data).map_err(|e| {
        // TODO: framed_recv_blocking should probably return io::Error
        io::Error::new(io::ErrorKind::Other, e)
    })?;

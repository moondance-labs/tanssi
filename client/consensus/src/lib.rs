...
use sp_runtime::{DigestItem, traits::{Block as BlockT, Header as HeaderT, Member, NumberFor}};
use std::{fmt::Debug, convert::TryFrom, hash::Hash, marker::PhantomData, pin::Pin, sync::Arc};
use log::{debug, trace};
use futures::prelude::*;
	...
	Error: std::error::Error + Send + From<sp_consensus::Error> + 'static,
{
	...
	fn pre_digest_data(&self, slot: Slot, claim: &Self::Claim) -> Vec<sp_runtime::DigestItem> {
		vec![
            ...
            <DigestItem as NimbusCompatibleDigestItem>::nimbus_pre_digest(nimbus_primitives::NimbusId::from_slice(claim.as_ref()).unwrap()),
        ]
	}

...
use dancebox_runtime::DryRunApiV2;
use dancebox_runtime::{OriginCaller, PolkadotXcm};
use frame_support::dispatch::{CallableCallFor, RawOrigin};
...
#[test]
fn dry_run_call_runtime_api_polkadot_xcm() {
    ExtBuilder::default()
        .with_balances(vec![
            (
                cumulus_primitives_core::relay_chain::AccountId::from(ALICE),
        ...
        .execute_with(|| {
            ...
            let origin: OriginCaller =
                OriginCaller::system(RawOrigin::Signed(AccountId::from(ALICE)));
            let xcm_version = 3;

            let beneficiary = VersionedLocation::V3(xcm::v3::MultiLocation {
                parents: 0,
                interior: xcm::v3::prelude::X1(xcm::v3::prelude::Junction::AccountId32 {
                    network: None,
                    id: [0x11; 32],
                }),
            });
            let pallet_balances_index = 10;
            let assets = VersionedAssets::V3(xcm::v3::MultiAssets::from(vec![MultiAsset {
                id: xcm::v3::AssetId::Concrete(xcm::v3::MultiLocation {
                    parents: 0,
                    interior: xcm::v3::prelude::X1(xcm::v3::prelude::Junction::PalletInstance(
                        pallet_balances_index,
                    )),
                }),
                fun: xcm::v3::Fungibility::Fungible(1_000_000_000_000_000),
            }]));
            let dest = VersionedLocation::V3(xcm::v3::MultiLocation {
                parents: 1,
             ...
             */
            let tx = RuntimeCall::PolkadotXcm(
                CallableCallFor::<PolkadotXcm, Runtime>::transfer_assets {
                    dest: Box::new(dest),
            ...
            // TODO: this gives error=DestinationUnsupported here because we dont mock the xcm stuff here
            // The typescript test fails with error=Transport("Other")
            let res = Runtime::dry_run_call(origin, tx, xcm_version).unwrap();

            println!("{:?}", res);

            assert!(res.execution_result.is_ok());
        });
}

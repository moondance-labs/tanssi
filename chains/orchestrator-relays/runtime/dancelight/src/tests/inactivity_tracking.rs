    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                ...
                get_active_collators_for_current_session(vec![ALICE.into(), BOB.into()])
            );
            assert_eq!(
            ...
            note_block_authors(vec![
                (ALICE.into(), 3000.into()),
                (CHARLIE.into(), 3001.into()),
            ]);
            assert_eq!(
                <ActiveCollatorsForCurrentSession<Runtime>>::get(),
                get_active_collators_for_current_session(vec![
                    ALICE.into(),
                    BOB.into(),
                    CHARLIE.into()
                ])
            ...
            assert_eq!(
                <ActiveCollatorsForCurrentSession<Runtime>>::get(),
                get_active_collators_for_current_session(vec![CHARLIE.into()])
            );
            assert_eq!(
                <ActiveContainerChainsForCurrentSession<Runtime>>::get(),
            ...
            run_block();
            note_block_authors(vec![(BOB.into(), 3000.into())]);
            assert_eq!(
                <ActiveCollatorsForCurrentSession<Runtime>>::get(),
                get_active_collators_for_current_session(vec![BOB.into(), CHARLIE.into()])
            );
            assert_eq!(
                <ActiveContainerChainsForCurrentSession<Runtime>>::get(),
                get_active_chains_for_current_session(vec![ParaId::from(3000)])

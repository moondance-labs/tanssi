...
fn basic_setup_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(ExternalValidators::invulnerables(), vec![1, 2]);
    });
}
...
fn add_invulnerable_works() {
    new_test_ext().execute_with(|| {
        ...
        // function runs
        assert_ok!(ExternalValidators::add_invulnerable(
        ...
        assert_noop!(
            ExternalValidators::add_invulnerable(RuntimeOrigin::signed(RootAccount::get()), new),
            Error::<Test>::AlreadyInvulnerable
        );

        // new element is now part of the invulnerables list
        assert!(ExternalValidators::invulnerables().to_vec().contains(&new));

        // cannot add with non-root
        assert_noop!(
            ExternalValidators::add_invulnerable(RuntimeOrigin::signed(1), new),
            BadOrigin
        );
    });
}
    ...
    new_test_ext().execute_with(|| {
        ...
        for ii in 3..=21 {
            if ii < 21 {
                ...
                assert_noop!(
                    ExternalValidators::add_invulnerable(
                        RuntimeOrigin::signed(RootAccount::get()),
                        ii
                    ),
        ...
        assert_eq!(ExternalValidators::invulnerables(), expected);
    });
}
...
fn remove_invulnerable_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(ExternalValidators::invulnerables(), vec![1, 2, 4, 3]);

        assert_ok!(ExternalValidators::remove_invulnerable(
            RuntimeOrigin::signed(RootAccount::get()),
            2
        ));
        ...
        assert_eq!(ExternalValidators::invulnerables(), vec![1, 4, 3]);

        // cannot remove invulnerable not in the list
        assert_noop!(
            ExternalValidators::remove_invulnerable(RuntimeOrigin::signed(RootAccount::get()), 2),
            Error::<Test>::NotInvulnerable
        );

        // cannot remove without privilege
        assert_noop!(
            ExternalValidators::remove_invulnerable(RuntimeOrigin::signed(1), 3),
            BadOrigin
        );
    });
}

...
declare module "@polkadot/api-base/types/consts" {
    interface AugmentedConsts<ApiType extends ApiTypes> {
        babe: {
            /**
             * The amount of time, in slots, that each epoch should last. NOTE: Currently it is not possible to change the
             * epoch duration after the chain has started. Attempting to do so will brick block production.
             */
            epochDuration: u64 & AugmentedConst<ApiType>;
            /**
             * The expected average block time at which BABE should be creating blocks. Since BABE is probabilistic it is not
             * trivial to figure out what the expected average block time should be based on the slot duration and the
             * security parameter `c` (where `1 - c` represents the probability of a slot being empty).
             */
        ...
        beefy: {
             ...
             *
             * Since the `SetIdSession` map is only used for validating equivocations this value should relate to the bonding
             * duration of whatever staking system is being used (if any). If equivocation handling is not enabled then this
             * value can be zero.
             */
        ...
        convictionVoting: {
             ...
             *
             * Also used to compute weight, an overly large value can lead to extrinsics with large weight estimation: see
             ...
             *
             * It should be no shorter than enactment period to ensure that in the case of an approval, those successful
             * voters are locked into the consequences that their votes entail.
             */
        ...
        fellowshipReferenda: {
             ...
             * reads/writes needed for smaller voters, but also result in delays to the automatic referendum status changes.
            ...
            /**
             * The number of blocks after submission that a referendum must begin being decided by. Once this passes, then
             * anyone may cancel the referendum.
             */
        ...
        multiBlockMigrations: {
             ...
             *
             * A good default needs to selected such that no migration will ever have a cursor with MEL above this limit. This
             * is statically checked in `integrity_test`.
             */
            cursorMaxLen: u32 & AugmentedConst<ApiType>;
            /**
             * The maximal length of an encoded identifier.
             *
             * A good default needs to selected such that no migration will ever have an identifier with MEL above this limit.
             * This is statically checked in `integrity_test`.
             */
        ...
        registrar: {
            ...
            /**
             * The deposit to be paid to run a on-demand parachain. This should include the cost for storing the genesis head
             * and validation code.
             */
        ...
        scheduler: {
             ...
             * NOTE:
             *
             * - Dependent pallets' benchmarks might require a higher limit for the setting. Set a higher limit under

...
impl ContainerChainSpawner {
    ...
    pub async fn rx_loop(self, mut rx: mpsc::UnboundedReceiver<CcSpawnMsg>, end_lookahead_sender: tokio::sync::watch::Sender<()>) {
    ...
    async fn handle_update_assignment(&self, current: Option<ParaId>, next: Option<ParaId>, end_lookahead_sender: &tokio::sync::watch::Sender<()>) {

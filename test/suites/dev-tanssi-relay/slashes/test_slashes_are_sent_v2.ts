...
import { generateBabeEquivocationProof, jumpToSession, USE_V2_STORAGE_KEY } from "utils";
import { STARLIGHT_VERSIONS_TO_EXCLUDE_FROM_SNOWBRIDGE_V2 } from "helpers";

describeSuite({
    id: "DEVT1710",
    title: "Babe slashes are sent after defer period in v2",
    ...
    testCases: ({ it, context }) => {
        ...
        it({
            id: "E01",
            title: "Babe offences should be confirmed after defer period in v2",
            test: async () => {
                ...
                const primaryChannelNonceBefore = await polkadotJs.query.ethereumOutboundQueueV2.nonce();

                await context.createBlock();
                const expectedUnprocessedMessagesAfterOneBlock =
                    await polkadotJs.query.externalValidatorSlashes.unreportedSlashesQueue();
                const primaryChannelNonceAfter = await polkadotJs.query.ethereumOutboundQueueV2.nonce();

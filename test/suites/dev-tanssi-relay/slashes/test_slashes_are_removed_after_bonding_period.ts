    ...
    testCases: ({ it, context }) => {
        ...
        it({
            ...
            test: async function () {
                ...
                let bondingPeriod = await polkadotJs.consts.externalValidatorSlashes.bondingDuration;

                let currentIndex = await polkadotJs.query.session.currentIndex();
                console.log("bondingPeriod is ", bondingPeriod);
                console.log("currentIndex is ", currentIndex);
                console.log("sessionsPerEra is ", sessionsPerEra);

                let targetSession = currentIndex.toNumber() + sessionsPerEra*(DeferPeriod +1) + sessionsPerEra*bondingPeriod;
                // TODO: check this

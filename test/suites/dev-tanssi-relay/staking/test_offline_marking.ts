...
describeSuite({
    ...
    testCases: ({ it, context }) => {
        ...
        async function isAddressInAssignedCollators(address: string) {
            const assignedCollatorsRecords = await polkadotJs.query.tanssiCollatorAssignment.collatorContainerChain();
        ...
        beforeAll(async () => {
            ...
            const removeBobFromInvulnerablesTx = polkadotJs.tx.sudo.sudo(
                polkadotJs.tx.tanssiInvulnerables.removeInvulnerable(bob.address)
            );
            ...
            expect((await polkadotJs.query.tanssiInvulnerables.invulnerables()).includes(bobAccountId)).to.be.false;
        ...
        it({
            ...
            test: async () => {
                ...
                const stakingCandidates = await polkadotJs.query.pooledStaking.sortedEligibleCandidates();
                expect(stakingCandidates.toJSON()).to.deep.equal([]);
                expect(await isAddressInAssignedCollators(bob.address)).to.be.false;
            },
        });
    },
});

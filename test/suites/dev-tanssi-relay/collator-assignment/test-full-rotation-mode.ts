    ...
    testCases: ({ it, context }) => {
        ...
        it({
            ...
            test: async function () {
                ...
                const tx = context.polkadotJs().tx.collatorConfiguration.setFullRotationMode(orchestrator, parachain, parathread);
                await context.createBlock(polkadotJs.tx.sudo.sudo(tx).signAsync(alice));

                const fullRotationPeriod = (await polkadotJs.query.collatorConfiguration.activeConfig())[
                    "fullRotationPeriod"
                ].toString();

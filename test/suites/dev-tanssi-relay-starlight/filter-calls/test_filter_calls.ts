...
describeSuite({
    id: "DEVT2001",
    title: "Starlight: Filter calls test",
    ...
    testCases: ({ it, context }) => {
            ...
            test: async () => {
                if (specName === "starlight" && specVersion >= 1300) {
                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.ethereumSystem.createAgent().signAsync(alice)
                    );

                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.ethereumOutboundQueue.setOperatingMode("Halted").signAsync(alice)
                    );

                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.ethereumInboundQueue.setOperatingMode("Halted").signAsync(alice)
                    );

                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.ethereumBeaconClient.setOperatingMode("Halted").signAsync(alice)
                    );

                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.ethereumTokenTransfers
                            .transferNativeToken(12345n, "0x0000000000000000000000000000000000000001")
                            .signAsync(alice)
                    );
                }
            },
            ...
            test: async () => {
                if (specName === "starlight" && specVersion >= 1300) {
                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.pooledStaking.updateCandidatePosition([]).signAsync(alice)
                    );
                }
            },
        });
        ...
        it({
            id: "E04",
            title: "Registrar calls are filtered",
            test: async () => {
                if (specName === "starlight" && specVersion >= 1300) {
                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.registrar.register(2000, "0x", "0x").signAsync(alice)
                    );

                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.registrar.deregister(2000).signAsync(alice)
                    );
                }
            },
        });
        ...
        it({
            id: "E05",
            title: "ContainerRegistrar (parathread) calls are filtered",
            test: async () => {
                if (specName === "starlight" && specVersion >= 1300) {
                    await checkCallIsFiltered(
                        context,
                        polkadotJs,
                        polkadotJs.tx.containerRegistrar.registerParathread(2000, "0x", "0x", "0x").signAsync(alice)
                    );
                }
            },
        });
...
async function checkCallIsFiltered(context: any, polkadotJs: ApiPromise, tx: any) {
    try {

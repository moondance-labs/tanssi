    ...
    beforeAll(async function () {
      ...
      const getBlockData = async (blockNum: number) => {
        ...
        return {
          ...
          authorities: await apiAt.query.authorityAssignment.collatorContainerChain(await apiAt.query.session.currentIndex())
        };
      };
    ...
    it({
      ...
      test: async function () {
        ...
        .map(({ blockNum, logs, authorities }) => {
          ...
          const nimbusLog = logs.filter(log => 
          ...
          const slotLog = logs.filter(log => 
            log.isPreRuntime === true && log.asPreRuntime[0].toHex() == stringToHex('aura')
          );
          const slot = slotLog[0].asPreRuntime[1].reverse().toHex();
          
          const orchestratorAuthorities = authorities.toJSON()["orchestratorChain"];
          const expectedAuthor = orchestratorAuthorities[hexToNumber(slot) % orchestratorAuthorities.length];

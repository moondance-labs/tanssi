...
describeSuite({
  ...
  testCases: ({ context, log, it }) => {
    ...
    for (const txnType of TransactionTypes) {
      it({
        ...
        test: async function () {
          ...
          const raw = await createRawTransfer(context, ALITH_ADDRESS, 10_000_000_000_000_000_000n - 21000n * gasPrice, {
            privateKey,
            type: txnType,
            gasPrice: gasPrice,
            gas: 21000n,
    ...
    it({
      ...
      test: async function () {
        const randomAccountBalance = await context.viem("public").getBalance({ address: randomAccount.address });

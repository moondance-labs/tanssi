...
describeSuite({
    ...
    testCases: ({ it, context }) => {
            ...
            test: async () => {
                ...
                for (const log of logs) {
                    ...
                    await checkLogsNotExist(logFilePath, [
                        ...
                        // Because of a race condition we can see this log sometimes, as DHT peer discovery is not
                        // instant. But it does get fixed.
        ...
        it({
            id: "T22",
            title: "Check collator logs for bootnodes found",
            test: async () => {
                // Assert that all collators have found bootnodes, and no collators failed to find bootnodes
                const logs = [
                    "/Collator-01.log",
                    "/Collator-02.log",
                    "/Collator-03.log",
                ...
                for (const log of logs) {
                    ...
                    await checkLogs(logFilePath, [
                        "Embedded DHT bootnode enabled: false; DHT bootnode discovery enabled: true",
                    ...
                    await checkLogsNotExist(logFilePath, [
                        "Failed to discover parachain bootnodes on the relay chain DHT",
                    ]);
                }
            },
        });

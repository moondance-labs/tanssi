...
export async function expectSystemEvent(
    ...
    expect(foundEvents, `Event ${section.toString()}.${method.toString()} appeared multiple times`).to.be.length(1);
    expect(event, `Event ${section.toString()}.${method.toString()} not found in block ${blockHash}`).to.not.be
        .undefined;
    return event;
}
